...
struct SustainTraining: View {
    ...
    @State private var totalDuration = 30.0
    ...
    @State private var scrollDuration: Double = 60.0
...
//                                    print("interpolatedY: \(interpolatedY)")
                ...
                if let midiURL = Bundle.main.url(forResource: "sustain", withExtension: "mid") {
...
//                    highlights = mapEventsToGrid(events)
                    ...
                    if let lastNote = events.max(by: { $0.time < $1.time }) {
                        ...
                        let preferredColumnDuration = 10.0 // 🧠 1 kolom = 0.2 detik → lebih pelan
                            timePerColumn = preferredColumnDuration
                            scrollDuration = midiLength * 10
...
//                        playSoundAudio()
            ...
            .navigationDestination(isPresented: $shouldNavigate) {
                SustainCompleted(history: $history, path: $path) // <- replace with your actual destination view
            }
        }
    ...
    func plays(){
        do {
            ...
            Timer.scheduledTimer(withTimeInterval: 1.0 / 600.0, repeats: true) { timer in
    ...
    func playSoundAudio() {
        guard let url = Bundle.main.url(forResource: "sustains", withExtension: "mp3") else { return }
        do {
